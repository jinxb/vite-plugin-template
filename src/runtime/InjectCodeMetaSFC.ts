  ...
  const DFS = (node: any, index: string, slotName?: string) => {
    ...
    for (let i = validChildren.length - 1; i >= 0; i--) {
      ...
      DFS(
        validChildren[i],
        `${
          validChild.tagType === 2 || validChild.tagType === 3
            ? index
            : `${index}.${i}`
        }`,
        slotName,
...
const InjectCodeMetaSFC = (source: string, filename = 'app'): string => {
...
export default InjectCodeMetaSFC;
